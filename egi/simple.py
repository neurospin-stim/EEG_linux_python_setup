...
class Eggog(Exception):
    ...
    def check_type(string_key):
        ...
        if type(string_key) != type(""):
            ...
            raise Eggog(
                "'%s': EGI wants the key to be four _characters_ (not %s) !"
                % (type(string_key),)
            )
    ...
    def check_len(string_key):
        ...
        if len(string_key) != 4:
            ...
            raise Eggog(
                "'%s': EGI wants the key to be exactly four characters!"
                % (string_key,)
            )
    ...
    def try_as_int(i):
        ...
        try:
            ...
            raise Eggog(
                "'%s': failed to convert to a (Python) integer!" % (repr(i),)
            )
...
def ms_localtime(warnme=True):
    ...
    if warnme and (ms_remainder < _ts_last):
        ...
        raise Eggog(
            "internal 32-bit counter passed through zero, please resynchronize ( call .synch() once again )"
        )
    ...
    return (
        ms_remainder
    )  # finish the recording before midnight ( and start after 00:00 )
...
class _Format:
    ...
    def pack(self, key, *args):
        ...
        result = struct.pack(fmt, key.encode('ascii'), *args)
...
def pstring(s):
    ...
    ps = struct.pack(fmt, s.encode('ascii'))
...
class _DataFormat:
    ...
    def __init__(self):
        ...
        self._check_table = {  # type(1L) :  lambda x : int(x) , # if this fails then the next attempt would have probably been do
    ...
    def _make_event_header(
        self, size_of_the_rest, timestamp, duration, keycode
    ):
        """
        ...
        result_str = struct.pack(
            "=sH2L4s", "D".encode('ascii'), total_length, timestamp, duration, keycode.encode('ascii')
        )
    ...
    def pack(
        ...
        if not is_32_bit_int_compatible(timestamp):
            ...
            raise Eggog(
                "only 'small' 32-bit integer values less than %d are accepted as timestamps, not %s"
...
class Netstation:
    ...
    def GetServerResponse(self, b_raise=True):
        ...
        code = self._socket.read(1).decode('ascii')
        ...
        if code == "Z":
            ...
            return (
                self._egi_protocol_version
            )  # just a bit more informative than 'None'
    ...
    def BeginSession(self):
        ...
        # print("sending")
        # print(self._socket)
        # self._socket.write( 'Q%s' % ( systemSpec, )  )
        # print("sent")
    ...
    def send_event(
        ...
        message = self._data_fmt.pack(
            key, timestamp, label, description, table, pad
        )
        self._socket.write(message)
    ...
    def send_timestamped_event(
        self, key, label=None, description=None, table=None, pad=False
    ):
        """
    ...
    def SendSimpleEvent(self, markercode, timestamp=None):
        ...
        data_string = "D%s%s%s%s" % (
            struct.pack(
                "h", event_min_size
            ),  # using 'default', or "native", endianness
